...
def convert_refinement_screening(refinement_anomalies, odir, base_name_wo_ext):
    formatted_data = [
        {"timestamp": ts, "value": round(val, 15)}
        for ts, val in refinement_anomalies
    ]

    saved_file_wo_ext = os.path.join(odir, base_name_wo_ext)

    output_path = Path(saved_file_wo_ext+"_refscr.json")
    output_path.write_text(
        json.dumps(formatted_data, indent=2, ensure_ascii=False)
    )