...
class ContainerCPU:
    ...
    def __init__(self, metrics_result_file, output_dir, zscore_threshold, window_size, watermark):
        ...
        try:
            ...
            self.timestamps = timestamps = [d['timestamp'] for d in self.data]
    ...
    def refinement_screening(self):
        ...
        """
        try:
            self.refined_anomalies = algo.moving_window_statistics(self.preliminary_anomalies,
                self.timestamps,
                self.values,
                self.window_size,
                self.zscore_threshold)
    ...
    def final_screening(self):
        try:
            self.final_anomalies = algo.watermark(self.refined_anomalies,
                self.watermark)
        except BaseException as re:
